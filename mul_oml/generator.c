...
run(opts_t *opts, oml_mps_t *oml_mps)
{
  ...
  oml_inject_switch(oml_mps->switch,"Switch-DP-id    |   State     |  Peer                 | Ports");
  oml_inject_switch(oml_mps->switch,"-----------------------------------------------------------------------------");

  b = mul_get_switches_brief(cli->mul_service);
  if (b) {
      pbuf = mul_dump_switches_brief(b, true);
      if (pbuf) {
          oml_inject_switch(oml_mps->switch,pbuf);
          free(pbuf);
      }    
  }
  ...
  do {
    ...
    if(oml_inject_switch(oml_mps->switch, "foo") != 0) {
      logwarn("Failed to inject data into MP 'switch'\n");
    }
    if(oml_inject_flow(oml_mps->flow, "foo") != 0) {
      logwarn("Failed to inject data into MP 'flow'\n");
    }
